...
function Query (criteria, options) {
  ...
  this._pipeline = proto._pipeline || undefined;
...
Query.prototype.sum = function (property, callback) {
    ...
    {
        this._pipeline = [{$group: {_id: null, total: {$sum: property}}}];
    }
    ...
    var cb_replacement = function(err, data) {
        if (err){return callback(err);} else if(data.constructor !== Array) { return callback(err, data);} callback(err, data[0]['total']);};
