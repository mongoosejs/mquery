...
Query.prototype.collection = function collection (coll) {
  ...
  var self = this;
  function getVersion() {
    // don't bother checking again if we already have it
    if (Query.__mongodVersion) return;
    var db = self._collection.collection.db;
    db.admin(function(err, admin) {
      admin.serverStatus(function (err, info) {
        var version = info.version.split('.').map(function(n){return parseInt(n, 10) });
        Query.__mongodVersion = version;
      });
    });
  }
  if (this._collection.collection.db._state == 'connected') {
    // we are already connected to the db, get the current version
    getVersion();
  } else {
    this._collection.collection.db.once('open', function () {
      getVersion();
    });
  }
...
Query.prototype.within = function within () {
  ...
  if (Query.__mongodVersion && Query.__mongodVersion[0] >= 2 && Query.__mongodVersion[1] >= 4) {
...
Query.prototype._updateForExec = function () {
  var ops = utils.keys(this._update)
    , i = ops.length
    , ret = {}
    , hasKeys
    , val
  ...
  while (i--) {
    ...
    if ('$' !== op[0]) {
      ...
      if (!ret.$set) {
        if (this._update.$set) {
          ret.$set = this._update.$set;
        } else {
          ret.$set = {};
        }
      }
      ret.$set[op] = this._update[op];
      ops.splice(i, 1);
